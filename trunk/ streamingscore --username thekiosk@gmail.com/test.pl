...
my $count=0;
my $access_token = 'AAACEdEose0cBAPvadSxemoaZAe0o4WjtxasWZChKm0B2ZBvCiL5ZAqKriSwSozxhXAUP7pMv51c71bA9QzlERHDyyZAVLd0yxewDZBGZA2PQwZDZD';
...
$browserGet->proxy('http', "http://wtd.ime.reuters.com/proxy.pac");
open(IN, "test.html")   or die "Can't open input.txt: $!";
@STRINGTEMP = <IN>;
close IN;
my $fullString="";
foreach my $tmpString (@STRINGTEMP){
	$fullString = $fullString.$tmpString;
}

@STRINGTEMP = split /(?:XXXXXXXXXX)/, $fullString; 

foreach my $temp (@STRINGTEMP){
#$filename = "debug.log"; open FH, ">".$filename or die "could not open $filename: $!\n";
...
@body = ($temp =~ /<table width="468" bgcolor="#666666" cellspacing="0" cellpadding="0" border="0"><tr bgcolor="#111111"><td colspan="4" height="4">(.*?)<\/table>/);
	...
	foreach my $key1 (keys %hHistoryScore) {
		#foreach my $key2 (keys %hStreamingScore) {
			...
			if (("$pervious" ne "$update") && ("$update" ne "00") && ("$update" ne "") && ("$pervious" ne "") && ($sumNew > 0) && ($sumNew > $sumOld) && ($time ne "FT") && ($time ne "HT")){
				$isUpdate = "TRUE";
	...
	if ($count ne 0){
		if ($isUpdate eq "TRUE"){						
			...
			while (<INFILE>) {
				...
				$tempString[1] =~ s/\n//g;
				push ( @{$hCommentData{"$tempString[0]"}}, $tempString[1]);
			}
			close INFILE;
			...
			foreach my $newLine (@realTimeScore){
				...
				#print "$newLine\n";
				if (@{$hCommentData{"$stringKey[1]"."|"."$stringKey[3]"}}[0]){
					print "Update on data Block ".@{$hCommentData{"$stringKey[1]"."|"."$stringKey[3]"}}[0]."\n";
					$get = $browserPost->post('https://graph.facebook.com/'.@{$hCommentData{"$stringKey[1]"."|"."$stringKey[3]"}}[0].'/comments',
					[
						access_token => $access_token,
						message      => $stringKey[0]."\t".$stringKey[1]."\t".$stringKey[2]."\t".$stringKey[3]."\n".$stringKey[4],
						name         => 'Post to your own Facebook account from a script',
						description  => 'This score has been retrieve date from http://livescore.com '
					...
					if ($commentID[0] ne ""){
						print "Create New data Block ".$commentID[0]."\n";
						push ( @{$hCommentData{"$stringKey[1]"."|"."$stringKey[3]"}}, $commentID[0]);
					}
				}				
